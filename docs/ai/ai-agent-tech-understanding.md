---
title: "AI Agent 技术理解：从 LLM、RAG 到 Agent 架构与 AI 测试体系"
description: 系统梳理 LLM、RAG、AI Agent 技术原理与 AI 功能测试、AI 效果评测、AI 鲁棒性测试、安全测试的工程化要点，覆盖模型评估方法与 AI 测试提效实践
category: AI
tags:
  - AI Agent
  - LLM
...
因此，理解 AI 技术原理不是测试人员的"附加技能"，而是做好 AI 测试的前置条件。只有理解了 Token、上下文窗口、采样参数、幻觉来源，才能设计出真正有效的测试用例；只有理解了 RAG 的检索与重排机制，才能定位"回答错误"的根因是检索召回不足还是生成阶段失真。
...
这个闭环正是上下文工程在测试领域的落地：Agent 在每一步获取的代码、需求、测试结果和工具反馈，都构成它下一步行动的上下文。上下文质量决定 Agent 测试闭环的可靠性。
...
- [ ] 理解上下文窗口的含义，知道系统Prompt + 对话历史 + 检索结果如何占用窗口
...
- [ ] 对输入进行等价类划分，覆盖有效、无效、边界和对抗输入
- [ ] 为每个功能定义输出契约（格式、Schema、内容约束），而非依赖精确匹配
...
- [ ] 根据任务类型选择合适的指标（检索用召回率，生成用 ROUGE/BLEU，代码用 pass@k）
- [ ] 建立人工标注规范，包含评分维度、标准和示例，并校验标注一致性
- [ ] 搭建 LLM-as-a-Judge 自动评测流水线，作为人工评测的扩展
...
- [ ] 测试空输入、超长输入、特殊字符和注入尝试等边界输入
- [ ] 验证分布偏移场景（领域迁移、时间偏移）下的效果退化
- [ ] 设计长对话测试，验证上下文遗忘和累积误差
- [ ] 执行并发、限流和降级压力测试，验证系统稳定性

### 5. AI 安全测试

- [ ] 测试直接 Prompt 注入和间接注入（通过检索内容）两种攻击路径
...
- [ ] 实现 Impact-based 回归筛选，优先运行高价值用例
- [ ] 把 AI 审查 + 自动化测试集成为质量门禁，嵌入 CI/CD
...
两条线不是平行的，而是相互支撑的：理解技术原理才能设计有效测试，系统化测试才能让技术能力稳定落地。
